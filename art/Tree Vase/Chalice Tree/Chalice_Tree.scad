// CHALICE TREE - a small vase glass (80 x 130 mm, its foot curving in over the
// bottom 17.5 mm) cupped 85 mm up in a 3 -> 7 -> 13 tree, printed in one piece
// on a 220 x 220 x 220 mm printer (Flashforge Adventurer 5 Pro; STL).
// Where the earlier trees pass their limbs flat under the glass and bend them up
// round its foot, here the three limbs rise out of the trunk in one smooth cup,
// never turning more than about 45 degrees from upright, and meet the glass on
// its curved foot: the glass sits down into them, on the profile of the
// branches, like an egg in a cup. There is no floor and no clearance: the wood
// grows against the glass itself (a tight fit), pressed flat where it touches.
// The limbs fork into 7 branches and those into 13, each turning its own way
// round the glass; wherever two meet, the later one arches out over the other
// and they grow together (arch_merge, 10 % planned, 30 % at most). The trunk
// comes down in buttresses that run out as roots over 200 mm of ground; trunk,
// buttresses and root bases are one skin with seamless handovers (guide 17).
// Every path is unique and nothing is symmetrical.
// sketch = true drops the bark, knots and fine sampling but keeps every path:
// use it while the shape is being found, then switch it off for the bark.
...
glass_foot_z = 17.5;    // the side starts curving in this far above the bottom
glass_foot_R = 50;      // radius of that curve (the bottom is then 73.7 mm across)
glass_foot = 2;         // small rounding of the bottom edge
glass_wall = 3;         // preview only
glass_bottom = 6;       // preview only
show_glass = true;

/* [2 - Fit and height] */
clearance = 0;          // radial gap around the glass: 0 = a tight fit, the wood touches the glass
lift = 85;              // glass bottom above the ground: where it sits on the branches
total_h = 215;          // overall height of the print, branch tips included (printer: 220)
...
root_count = 9;
root_spread = 200;      // diameter the roots reach over the ground (printer bed: 220)
...
wander_seed = 8;        // another number gives every branch a different wander
...
tip_open = 40;          // the tips open out from the glass over their last this-many mm
// The layout came from a search over random 3 -> 7 -> 13 trees
// (Chalice_Tree_layout.py; the seed and count are below) for the smallest empty
// sector around the glass at every height, a steady three-point seat and no
// stacked arches.
...
//  top (mm), how far the tip opens out from the glass (mm), meander amplitude
//  (deg), meander wavelength (mm), meander phase (deg), twig stubs (0..2)]
// Layout: Chalice_Tree_layout.py 58 4000 (best of 8 seeds x 4000 trees; largest empty sector 93 deg, seat 130 deg)
limb_table = [
    [ 347,  -84,   60,   56, 1.32,    0,    5,    4,  145,   69,    2],
    [ 117,  -70,    0,    0, 1.26,    6,    6,    4,  102,   85,    2],
    [ 224,   83,    0,    0, 1.36,    3,    5,    7,  103,  205,    2]];
// How each limb rises out of the trunk, so they part at different heights:
// [parting height (share of lift), where it settles onto the curved foot of the
//  glass (mm above the glass bottom, inside glass_foot_z), share of the rise
//  spent speeding up, share spent slowing down]. Limb 0 parts first; limbs 1 and
// 2 go on together as one forked stem, and limb 2 parts last. The limbs head for
// the curve of the foot, not under the bottom, so none of them has to turn
// through a right angle round the glass.
limb_rise = [[0.20, 10, 0.35, 0.5], [0.29, 12, 0.4, 0.5], [0.36, 15, 0.4, 0.55]];
...
fork_table = [
    [   2,   31,  -60,    0,    0, 0.7,    8,    3,    6,   96,  175,    1],
    [   1,   24,   58,    0,    0, 0.74,    6,    6,    6,  107,  284,    1],
    [   0,   16,   79,    0,    0, 0.65,    8,    3,    7,  142,  315,    1],
    [   2,   13,  -66,    0,    0, 0.66,    0,    5,    8,  140,  212,    1],
    [   0,   46,   89,   76,  -56, 0.64,    5,    5,    4,  117,   24,    1],
    [   5,   65,  -64,    0,    0, 0.66,   12,    3,    7,   83,  160,    0],
    [   4,   51,   56,    0,    0, 0.67,    5,    4,    4,  104,  284,    1],
    [   2,   61,  -80,    0,    0, 0.6,    7,    5,    6,   88,  194,    1],
    [   6,   63,   51,    0,    0, 0.69,    1,    6,    4,   99,  173,    1],
    [   3,   68,   78,    0,    0, 0.65,   11,    4,    6,  119,    0,    0]];
...
sketch = true;          // plain tubes, no bark or knots: the same paths and shapes, rendered in a
                        // fraction of the time, for finding the geometry before the surface

/* [Hidden] */
$fn = quality == 0 ? 48 : 96;
spp = sketch ? 3 : quality == 0 ? 5 : quality == 1 ? 8 : 12;   // surface samples per bark plate
...
bed = [220,220,220];    // printer build volume (Flashforge Adventurer 5 Pro)

assert(glass_d > 2*glass_wall && glass_h > glass_bottom, "Invalid glass dimensions");
assert(glass_foot_R >= glass_foot_z && glass_foot_z < glass_h, "The foot curve must fit the glass");
assert(lift >= 7.5, "The glass floor must be at least 7.5 mm above the plate");
assert(total_h <= bed[2], "Taller than the printer");
assert(root_spread <= min(bed[0],bed[1])-10, "Roots wider than the printer bed");
assert(total_h >= glass_top-40, "The branches must reach up the glass");
...
module bark_tube(P,R,depth,seed=0,knots=[],glass=false,wmix=undef) {
    ...
    Rmax=max(R); n=sketch ? 6 : max(5,round(2*PI*Rmax/plate_w)); S=n*spp;
...
// STEP 1: the glass, standing at the lifted height. Its side is straight down to
// glass_foot_z, then curves in along a circle of radius glass_foot_R to the
// bottom, whose edge is rounded by glass_foot.
function glass_prof(h,c=0) = h>=glass_foot_z ? glass_r+c
    : glass_r-glass_foot_R+sqrt(pow(glass_foot_R+c,2)-pow(glass_foot_z-max(0,h),2));
// The line the branches hug: the profile, carried on below the bottom along the
// tangent of the foot curve, so the limbs arriving from below have no kink where
// the glass ends.
foot_slope = glass_foot_z/sqrt(pow(glass_foot_R+clearance,2)-glass_foot_z*glass_foot_z);
function hug_prof(h) = h>=0 ? glass_prof(h,clearance) : glass_prof(0,clearance)+foot_slope*h;
glass_rb = glass_prof(0);       // radius of the bottom
module glass_vase() {
    hf=glass_foot; p1=[glass_prof(hf),hf]; t=unit([glass_prof(hf+0.1)-p1[0],0.1]);
    difference() {
        rotate_extrude($fn=96) polygon(concat([[0,0],[glass_rb-hf,0]],
            [for(k=[1:6]) let(u=k/6, c=p1-t*hf) (1-u)*(1-u)*[glass_rb-hf,0]+2*u*(1-u)*c+u*u*p1],
            [for(k=[1:12]) let(h=hf+(glass_foot_z-hf)*k/12) [glass_prof(h),h]],
            [[glass_r,glass_h],[0,glass_h]]));
        translate([0,0,glass_bottom]) cylinder(r=glass_r-glass_wall,h=glass_h);
    }
}

// The glass (grown by the clearance) as an obstacle, in (r, z) round the axis:
// from lift up, its curved foot and straight side, the bottom edge rounded. Wood
// that would grow into it is pushed out onto its surface along a smooth direction
// field (the same shape with a much rounder bottom edge, so the push never flips
// where a limb meets the foot); within press_k of it the push fades out smoothly.
env_e = glass_foot+clearance;   // rounding of the obstacle's bottom edge
env_eg = 12*sc;                 // rounding of the direction field's bottom edge
press_k = 2.5*sc;               // width of the smooth transition (mm)
foot_c = [glass_r-glass_foot_R,lift+glass_foot_z];     // centre of the foot curve
// Signed distance to the side (the curve, then the straight wall), moved in by e.
function side_d(r,z,e) = z>=foot_c[1] ? r-(bore_r-e) : norm([r,z]-foot_c)-(glass_foot_R+clearance-e);
function side_n(r,z) = z>=foot_c[1] ? [1,0] : unit([r,z]-foot_c);
function env_sdf(r,z) = let(x=side_d(r,z,env_e), y=lift+env_e-z)
    x>0 && y>0 ? norm([x,y])-env_e : max(x,y)-env_e;
function env_dir(r,z) = let(x=side_d(r,z,env_eg), y=lift+env_eg-z, n=side_n(r,z))   // [outward, up]
    x>0 && y>0 ? unit(x*n+[0,-y]) : x>y ? n : [0,-1];
// From a point inside, the distance along g to the obstacle's surface
// (bisection: the obstacle is convex, so the ray leaves it once).
function env_bis(r,z,g,lo,hi,n) = n==0 ? hi : let(m=(lo+hi)/2)
    env_sdf(r+m*g[0],z+m*g[1])<0 ? env_bis(r,z,g,m,hi,n-1) : env_bis(r,z,g,lo,m,n-1);
function env_exit(r,z,g) = env_bis(r,z,g,0,64,15);
...
// trunk, rise, part one after another, lean out in a cup and settle onto the
// curved foot of the glass, which sits down into them, then climb its side. The trunk is then one surface (radius as a function of
...
function branch_radius(z) = branch_r*lookup(z,[[branch_z0,1.1],[lift,1],[total_h,0.75]]);
// Centreline radius of a member of radius r pressed against the glass at height
// z: it follows the curved foot as well as the side.
function hug_r(r,z) = hug_prof(z-lift)+(1-press)*r;
// The tips open out from the glass over their last tip_open mm.
function lean(z,top,amount) = amount*pow(max(0,z-(top-tip_open))/tip_open,1.5);
branch_rho0 = 7*sc;
// Parting and out to the glass: a limb leaves the trunk upright, leans out at a
// steady rate and settles onto the curved foot of the glass, blending into the
// line it then follows up the glass (so there is no knee where it arrives). The speed rises
...
function tip_room(i) = min(1,tip_open/(70*sc));
function flick(i,z) = sprawl*sc*tip_room(i)*(4+6*wr(i,37))*(wr(i,38)<0.5 ? -1 : 1)
    *pow(max(0,z-(m_end(i)-tip_open))/tip_open,2);
...
module trunk() {
    zl=12*sc; n1=ceil(zl/(sketch ? 1 : 0.4)); n2=ceil((crotch_z-zl)/step); S=2*trunk_plates*spp;
    loft([for(z=concat([for(k=[0:n1]) zl*k/n1],[for(k=[1:n2]) zl+(crotch_z-zl)*k/n2]))
        ...
        [for(j=[0:S-1]) let(a=360*j/S, rc=trunk_rho(a,z,C),
            ds=sketch ? [] : [for(q=trunk_knots) norm([wrap(a-q[1])*PI/180*rc,z-q[0]])],
            calm=sketch ? 1 : min([for(m=[0:len(trunk_knots)-1]) knot_calm(ds[m],trunk_knots[m][2])]),
            bump=sketch ? 0 : sum([for(m=[0:len(trunk_knots)-1]) knot_h(ds[m],trunk_knots[m][2])]),
            rho=sketch ? rc : rc-bark_depth*calm*(1-wbark([rc*cos(a),rc*sin(a),z]))+bump)
...
function m_rho0(i,z,acc) = is_main(i)
    ? (z<hug_z(i) ? lerp(branch_rho0,m_rho_g(i,z),ease_out(i,z)) : m_rho_g(i,z))
...
function m_rho_g(i,z) = hug_r(m_r(i,z),z)+lean(z,m_end(i),Mb(i)[8])+wander_r(i,z);
...
function knot_zs(i) = pick(is_main(i) ? lift+25*sc : m_z0(i)+18*sc,min(glass_top,m_end(i))-15*sc,busy(i),is_main(i) ? 3 : 1,i+20);
function twig_zs(i) = pick(is_main(i) ? lift+32*sc : m_z0(i)+22*sc,min(glass_top,m_end(i))-15*sc,busy(i),Mb(i)[12],i+40,
...
echo(str("Chalice Tree | glass ",glass_d," x ",glass_h," mm, foot curving in from ",glass_foot_z,
    " mm (R ",glass_foot_R,", bottom d ",round(20*glass_rb)/10,") | bore d ",2*bore_r,
